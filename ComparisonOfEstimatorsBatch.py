...
# ============================================================
# BATCH MODE — set BATCH_MODE = True to loop over all configs
# ============================================================

BATCH_MODE = True   # flip to True to run all 12 experiments in sequence

# Each dict overrides the corresponding module-level global.
# Keys not listed here keep their default values from the CONFIG above.
BATCH_TABLE = [
    # Row 1 — true order (chronological datasets)
    {"DATA_SOURCE": "hamlet",          "TRUE_ORDER": True,  "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100},
    {"DATA_SOURCE": "butterflies",     "TRUE_ORDER": True,  "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100},
    {"DATA_SOURCE": "college_msg",     "TRUE_ORDER": True,  "GROUP_SUBSAMPLE_K": 100, "NUM_SHUFFLES": 100},

    # Row 2 — permutation averages
    {"DATA_SOURCE": "hamlet",          "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100},
    {"DATA_SOURCE": "butterflies",     "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100},
    {"DATA_SOURCE": "college_msg",     "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 100, "NUM_SHUFFLES": 100},

    # Row 3 — MTG with varying N_boosters
    {"DATA_SOURCE": "mtg",             "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100, "N_boosters": 12},
    {"DATA_SOURCE": "mtg",             "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100, "N_boosters": 24},
    {"DATA_SOURCE": "mtg",             "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,   "NUM_SHUFFLES": 100, "N_boosters": 48},

    # Row 4 — genes, synthetic
    {"DATA_SOURCE": "genes",           "TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 10,  "NUM_SHUFFLES": 100},
    {"DATA_SOURCE": "synthetic_uniform","TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,  "NUM_SHUFFLES": 100, "SYNTHETIC_M": 1000},
    {"DATA_SOURCE": "synthetic_uniform","TRUE_ORDER": False, "GROUP_SUBSAMPLE_K": 1,  "NUM_SHUFFLES": 100, "SYNTHETIC_M": 10000},
]
...
# ============================================================
# MAIN — wrapped as a function so the batch loop can call it
# ============================================================

def run_experiment():
    """Run one experiment using the current module-level config globals."""
    # DATA_SOURCE may be mutated for synthetic_uniform filenames;
    # CHEB_K is set inside the loop.
    global DATA_SOURCE, CHEB_K
    ...
    # 5) experiment
    estimator_keys = ["qsip", "SGT", "triv", "ratio", "fav",
    ...
    for s in range(num_runs):
        ...
        for j, n in enumerate(n_vals):
            ...
            if INCLUDE_QSIP and n in H_cache:
                ...
                _e = abs_pct_err(pred, V_true)
                mean["qsip"][j] += _e
                sum_sq["qsip"][j] += _e * _e

            if INCLUDE_SGT:
                inc = predict_new_SGT(phi, n, r)  # t := n
                _e = abs_pct_err(Sn + inc, V_true)
                mean["SGT"][j] += _e
                sum_sq["SGT"][j] += _e * _e
            ...
            if INCLUDE_CHEBYSHEV:
                ...
                _e = abs_pct_err(Sn + inc, V_true)
                mean["cheby"][j] += _e
                sum_sq["cheby"][j] += _e * _e

        print(f"Completed run {s+1}/{num_runs}")

    # 6) average over runs and compute standard deviations
    denom = float(num_runs)
    std = {}
    for k in mean:
        mean[k] /= denom
        sum_sq[k] /= denom                        # E[X^2]
        variance = sum_sq[k] - mean[k] ** 2        # Var = E[X^2] - (E[X])^2
        variance = np.maximum(variance, 0.0)        # guard against float rounding
        std[k] = np.sqrt(variance)
    ...
    if SAVE_RESULTS:
        ...
        payload = {
            ...
            "std_abs_pct_err": {k: std[k].tolist() for k in std},
        }
...
# ============================================================
# ENTRY POINT — batch or single run
# ============================================================

if __name__ == "__main__":

    if BATCH_MODE:
        print(f"=== BATCH MODE: {len(BATCH_TABLE)} experiments ===\n")
        for run_i, cfg in enumerate(BATCH_TABLE, 1):
            # --- override globals from this config row ---
            for key, val in cfg.items():
                globals()[key] = val

            ds_tag = cfg.get("DATA_SOURCE", DATA_SOURCE)
            to_tag = "TO" if cfg.get("TRUE_ORDER", TRUE_ORDER) else "perm"
            print(f"\n{'='*60}")
            print(f"  Batch run {run_i}/{len(BATCH_TABLE)}: {ds_tag} ({to_tag})")
            print(f"  Config: {cfg}")
            print(f"{'='*60}\n")

            run_experiment()

        print(f"\n=== ALL {len(BATCH_TABLE)} BATCH RUNS COMPLETE ===")

    else:
        # Single run using the module-level CONFIG values
        run_experiment()